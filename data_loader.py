...
class TrainDataset(Dataset):
	...
	def __getitem__(self, idx):
		...
		# get_item的输出：
		# triple: torch.LongTensor([1, 2, 3])
		# trp_label: (smoothed one-hot label) torch.FloatTensor([0.0, 0.0, 1.0, .....]) 长度为 entity_num
		return triple, trp_label, None, None, obj_list

	@staticmethod
	def collate_fn(data):
		obj_list = []
		for ele in data:
			obj_list = obj_list + ele[-1]
		# print(data[0])
		triple		= torch.stack([_[0] 	for _ in data], dim=0)
		trp_label	= torch.stack([_[1] 	for _ in data], dim=0)
		# triple: (batch_size, 3) torch.LongTensor
		# trp_label: (batch_size, ent_num) torch.FloatTensor
		return triple, trp_label, obj_list
